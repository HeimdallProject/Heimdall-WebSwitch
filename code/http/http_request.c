...
ThrowablePtr get_header(char *req_line, HTTPRequestPtr http) {
...
ThrowablePtr get_request(char *req_line, HTTPRequestPtr http, int len) {
...
ThrowablePtr get_response(char *req_line, struct http_request *http, int len) {
    ...
    for (i = 0; i < length; i++) {
        ...
        if (buffer[i] == endline) {
            ...
            if (start == 0) {
                switch (type) {
                    case RQST:
                        throwable = get_request(buffer, http, i);
                        if (throwable->is_an_error(throwable)) {
                            return throwable->thrown(throwable, "read_headers");
                        }
                        break;
                    case RESP:
                        throwable = get_response(buffer, http, i);
                        if (throwable->is_an_error(throwable)) {
                            return throwable->thrown(throwable, "read_headers");
                        }
                        break;
                ...
                throwable = get_header(buffer + start, http);
                if (throwable->is_an_error(throwable))
                    return throwable->thrown(throwable, "read_headers");
                start = i + 1;
            }
